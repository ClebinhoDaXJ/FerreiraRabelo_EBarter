...
Good::Good(World* mundo, string gname, vector<float> gqval, vector<float> gdval, vector<float> gnval, int initialQtd){
    world = mundo;
...
inline int Society::convertForeignCoins(){
    ...
    */
    ...
    for(unsigned i=0; i<foreignCoins.size(); i++){
        if(i != index){
            sum += (int)foreignCoins[i]*(world->societies[i]->cvalue)/cvalue;
            foreignCoins[i] = 0;
        }
    }
...
void World::gadd(string gname, vector<float> gqval, vector<float> gdval, vector<float> gnval, int initialQtd){ 
    for(int i=0; i<n_g; i++){
    ...
    Good* new_good = new Good(this, gname, gqval, gdval, gnval, initialQtd);
...
void World::grmv(string gname){
    ...
    for(unsigned i=0; i<n_g; i++){
        if(goods[i]->_gname == gname){
            ...
            societies[i]->sqv.erase(societies[i]->sqv.begin()+i);
            societies[i]->sdv.erase(societies[i]->sdv.begin()+i);
            societies[i]->snv.erase(societies[i]->snv.begin()+i);
            societies[i]->svv.erase(societies[i]->svv.begin()+i);
...
Operation::Operation(World* mundo, int numero_op){
    ...
    */
        for(int j=0; j<world->n_s; j++)
            ivv.push_back(world->societies[i]->cvalue);
        opMatix.push_back(ivv);
        ivv.clear();
...
inline int Operation::generateQtdC(){
    int cBv = buyer.cvalue;
    int cSv = seller.cvalue;
...
inline void Operation::atlzCoin(){
    ...
    */
    ...
    int qtd2 = qtd1*seller.cvalue/buyer.cvalue; //número de moedas cedidas pelo vendedor.
